...
import ru.otus.otuskotlin.incomingControl.app.biz.IctrlCommodityProcessor
...
interface ConsumerStrategy {
    ...
    fun serialize(source: IctrlContext): String
    fun deserialize(value: String, target: IctrlContext)
}
...
class AppKafkaConsumer(
    ...
    fun run() = runBlocking {
        try {
            ...
            while (process.value) {
                val ctx = IctrlContext(timeStart = Clock.System.now())
    ...
    private fun sendResponse(context: IctrlContext, strategy: ConsumerStrategy, outputTopic: String) {
